# Spam Detection with Logistic Regression (Midterm)
...
Upload the provided dataset to your repo:

- `data/o_tagviashvili25_61845.csv`

After you push, the GitHub link will look like:

`https://github.com/fellixdesuza/aimlmid2026_o_tagviashvili25/blob/main/data/o_tagviashvili25_61845.csv`
...
**Explanation (2–3 sentences):** This bar chart shows how many emails are labeled as legitimate vs. spam. If one class is much larger, the dataset is imbalanced and accuracy alone can be misleading. Here, the counts are close, so training is not strongly biased toward one class.
...
**Explanation (2–3 sentences):** This heatmap shows the number of correct and incorrect predictions on the test set. The diagonal cells (TN and TP) represent correct classifications, while off-diagonal cells (FP and FN) are errors. The small off-diagonal values indicate the model performs well on both classes.
...
**Explanation (2–3 sentences):** This chart shows the learned coefficient for each feature in Logistic Regression. Larger positive coefficients push the prediction toward spam (class 1). `capital_words` has the highest coefficient here, suggesting uppercase “shouting” words are a strong indicator of spam for this dataset.
